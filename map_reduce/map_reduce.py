...
class Orchestrator(object):
    ...
    def try_partition_to_reducer(self, key):
...
class MapReduce(object):
    ...
    def __init__(self, file_paths, map_func, reduce_func):
        ...
        self.orchestrator = Orchestrator(reduce_func)
        self.mapper = Mapper(map_func, self.orchestrator)
        self.map_tasks = []

    async def run(self):
        print('Initializing all mappers')
        self.instantiate_mappers()

        print('Waiting for map tasks to finish')
    ...
    def instantiate_mappers(self):
        for file_path in self.file_paths:
            ...
            print('Map worker created for: {}'.format(file_path))
